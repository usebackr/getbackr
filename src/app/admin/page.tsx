...
export default async function AdminDashboardPage() {
  ...
  const results = await Promise.allSettled([
    ...
    db.select({ count: sql<number>`count(*)::int` }).from(users).where(sql`is_beta = true`), // 1: betaCount
    db.select({ count: sql<number>`count(*)::int` }).from(users).where(sql`last_login_at > now() - interval '24 hours'`), // 2: dauCount
    ...
    db // 5: topCampaigns
      ...
      .limit(5),
    db.select({ count: sql<number>`count(*)::int` }).from(users).where(and(eq(users.isBeta, true), eq(users.emailVerified, true))), // 6: verifiedBeta
    db.select({ count: sql<number>`count(*)::int` }).from(users).where(and(eq(users.isBeta, true), eq(users.kycStatus, 'verified'))), // 7: kycBeta
  ...
  const getValue = (index: number) => results[index].status === 'fulfilled' ? (results[index] as any).value : null;

  const userCount = getValue(0)?.[0]?.count || 0;
  const betaCount = getValue(1)?.[0]?.count || 0;
  const dauCount = getValue(2)?.[0]?.count || 0;
  const campaignCount = getValue(3)?.[0]?.count || 0;
  const financialStats = getValue(4)?.[0] || null;
  const topCampaigns = getValue(5) || [];
  ...
  return (
    <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '24px', marginBottom: '32px' }}>
        <StatCard title="Total Beta Users" value={betaCount.toLocaleString()} color="#3b82f6" subValue={`${verificationRate}% verification rate`} />
        <StatCard title="Daily Active" value={dauCount.toLocaleString()} color="#f59e0b" subValue="Active last 24h" />
        <StatCard title="Platform Volume" value={`₦${totalVolume.toLocaleString()}`} color="#0f172a" isDark />
      ...
      <div style={{ 
        ...
        <h3 style={{ fontSize: '1.1rem', fontWeight: 800, color: '#0f172a', marginBottom: '24px' }}>Onboarding Funnel</h3>
        <div style={{ display: 'flex', gap: '32px', alignItems: 'center' }}>
          <FunnelStep title="Signups" count={betaCount} sub={`${verificationRate}% convert`} isActive />
          <FunnelArrow />
          <FunnelStep title="Verified" count={verifiedBetaCount} sub={`${kycRate}% move to KYC`} isActive={verifiedBetaCount > 0} />
          <FunnelArrow />
          <FunnelStep title="KYC Approved" count={kycBetaCount} sub="Ready to contribute" color="#10b981" isActive={kycBetaCount > 0} />
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: '32px', marginBottom: '48px' }}>
        ...
        <div style={{ background: '#fff', padding: '32px', borderRadius: '16px', border: '1px solid #e2e8f0', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)' }}>
          <h3 style={{ fontSize: '1.1rem', fontWeight: 800, color: '#0f172a', marginBottom: '24px' }}>Recent Beta Onboarding</h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            {recentBetaUsers.map((u: any) => (
              <div key={u.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingBottom: '12px', borderBottom: '1px solid #f1f5f9' }}>
                <div>
                  <div style={{ fontWeight: 700, fontSize: '0.9rem', color: '#0f172a' }}>{u.displayName || u.email}</div>
                  <div style={{ fontSize: '0.75rem', color: '#64748b' }}>Joined {new Date(u.createdAt).toLocaleDateString()}</div>
                </div>
...
function FunnelStep({ title, count, sub, isActive, color = '#3b82f6' }: any) {
  return (
    <div style={{ flex: 1, textAlign: 'center', opacity: isActive ? 1 : 0.4 }}>
      <p style={{ fontSize: '0.75rem', fontWeight: 800, color: '#64748b', textTransform: 'uppercase', marginBottom: '8px' }}>{title}</p>
      <div style={{ fontSize: '1.75rem', fontWeight: 900, color: isActive ? color : '#64748b' }}>{count}</div>

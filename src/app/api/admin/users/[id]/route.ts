  ...
  try {
    ...
    await db.transaction(async (tx) => {
      ...
      if (campaignIds.length > 0) {
        ...
        const wallets = await tx.select({ id: projectWallets.id }).from(projectWallets).where(inArray(projectWallets.campaignId, campaignIds));
      ...
      if (campaignIds.length > 0) {
        await tx.delete(contributions).where(
          or(
            eq(contributions.backerId, userId),
    ...
    return NextResponse.json({ error: 'Failed to delete user. Database transaction failed.' }, { status: 500 });
  }
}
